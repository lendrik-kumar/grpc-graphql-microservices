...
import (
	...
	"github.com/lendrik-kumar/graphql-grpc-go-microservices/order/pb"
...
func (c *Client) GetOrdersForAccount(ctx context.Context, accountID string) ([]Order, error) {
	...
	for _, orderProto := range r.Orders {
		...
		// fixed: UnmarshalBinary into addressable local, then assign
		createdAt := time.Time{}
		if err := createdAt.UnmarshalBinary(orderProto.CreatedAt); err != nil {
			return nil, err
		}
